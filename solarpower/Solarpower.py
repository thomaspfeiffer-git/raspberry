...
class CSV (object):
    ...
    def get_item_from_rrd (self, rrd_template, rrd_data, item):
    ...
    def write (self, rrd_template, rrd_data):
        ...
        csv_data = { "Timestamp": datetime.now().strftime("%Y%m%d %H:%M:%S") } |         \
                   self.get_item_from_rrd(rrd_template, rrd_data, Main_Meter.field_P) |  \
                   self.get_item_from_rrd(rrd_template, rrd_data, Solar_Meter.field_P)
        """
        csv_data = { "Timestamp": datetime.now().strftime("%Y%m%d %H:%M:%S") }
